...
temporal co-activity) are wired end to end (issues #12, #28): one
`POST /v1/investigations/traces` flow composes waterfall spans, related
logs, surrounding metrics and the correlated relations under one caller
budget. The investigation surfaces (Loom UI) and MCP are later milestones
of the phase.
