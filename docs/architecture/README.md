...
| [investigation-model.md](investigation-model.md) | The Investigation API and the shape of its answers (the `Investigation` envelope)                         |
| [query-model.md](query-model.md)                 | The query contract: budgets, ordering, cursors, and the refuse-or-degrade policy                          |
| [correlation-model.md](correlation-model.md)     | Relation types, tiers, evidence and provenance                                                            |
| [storage-model.md](storage-model.md)             | The storage abstraction, the two runtime storage modes, retention, and the ingestion↔persistence hand-off |
| [mcp-model.md](mcp-model.md)                     | The MCP boundary: what it shares with the UI and what it must never do                                    |
| [runtime-constraints.md](runtime-constraints.md) | Resource budgets (engineering targets) and the backpressure policy                                        |
...
configuration (`module-boundaries.config.mjs` at the repository root) must
stay in sync with `boundaries.md`; the configuration is
